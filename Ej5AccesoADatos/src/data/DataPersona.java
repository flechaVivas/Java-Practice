...
public class DataPersona {
	...
	public void update(Persona per) {
		...
		try {
			stmt=DbConnector.getInstancia().getConn().prepareStatement(
					"update persona set nombre=?, apellido=?, email=?, tel=?, password=? where tipo_doc=? and nro_doc=?");
			
			stmt.setString(1, per.getNombre());
			stmt.setString(2, per.getApellido());
			stmt.setString(3, per.getEmail());
			stmt.setString(4, per.getTel());
			stmt.setString(5, per.getPassword());
			stmt.setString(6, per.getDocumento().getTipo());
			stmt.setString(7, per.getDocumento().getNro());
